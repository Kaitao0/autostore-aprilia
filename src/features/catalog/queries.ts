...
/** Vehicles for the showroom/tablet and kiosk display. */
export async function getShowroomVehicles(): Promise<PublicVehicleRow[]> {
  if (!isSupabaseConfigured()) return [];
  try {
    const supabase = await createClient();
    const { data } = await supabase
      ...
      .in("availability", ["disponibile", "riservato", "in_arrivo"])
      .order("featured", { ascending: false })
      .order("published_at", { ascending: false, nullsFirst: false })
      .limit(60);
    return data ?? [];
...
export { vehicleTitle, vehicleBadges } from "./shared";
